...
class GatewayClient(
  private val onState: (connected: Boolean, registered: Boolean, sessionId: String?, lastError: String?) -> Unit,
  ...
  private val listener = object : WebSocketListener() {
    override fun onOpen(webSocket: WebSocket, response: Response) {
      ...
      onState(true, false, sessionId, null)
    ...
    override fun onFailure(webSocket: WebSocket, t: Throwable, response: Response?) {
      ...
      onState(false, false, sessionId, "WS_FAILURE")
      scheduleReconnect()
    }
    ...
    override fun onClosed(webSocket: WebSocket, code: Int, reason: String) {
      ...
      onState(false, false, sessionId, null)
      scheduleReconnect()
    }
  }
  ...
  private fun sendRegistration(webSocket: WebSocket) {
    ...
    onLog("TX $payload")
    logTranscript("TX", "node", "register", payload)
    webSocket.send(payload.toString())
  }

  private fun nodeInfo(): JSONObject {
    return JSONObject()
  ...
  private fun scheduleReconnect() {
    ...
    val reconnectPayload = JSONObject()
      .put("url", url)
      .put("attempt", attempt)
      .put("delaySeconds", backoffSeconds)
    logTranscript("TX", "node", "reconnect", reconnectPayload, "scheduled")
  ...
  private fun logTranscript(
    ...
    val transcript = JSONObject()
      ...
      .put("payload", payload)
    if (note != null) {
      transcript.put("note", note)
    }
    onLog("TRANSCRIPT ${transcript}")
  }
}
