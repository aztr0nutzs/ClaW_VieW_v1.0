...
/**
 * Foreground service responsible for managing the lifecycle of the [GatewayClient] and
 * processing commands from the UI. It owns the gateway and ensures that resources are
...
class OpenClawForegroundService : Service() {
  ...
  override fun onDestroy() {
    ...
    running.set(false)
    // fully shut down the gateway when the service is destroyed. This cancels reconnects,
    // closes the socket and releases OkHttp resources.
  ...
  companion object {
    ...
    fun getUiStateSnapshot(): UiState {
      ...
      return if (running.get()) {
        ...
        state.copy(nodeId = null, connected = false, registered = false)
      }
    }
